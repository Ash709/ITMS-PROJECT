...
def detect_vehicles():
    ...
    for i, cap in enumerate(videos):
        ...
        results = model.predict(frame, verbose=False)  # safer
        ...
        # Add vehicle count on frame
        cv2.putText(frame, f"Vehicles: {vehicle_count}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        frames[i] = frame

    # Update global counts & signal times
    with lock:
        lane_counts[:] = results_list
        total_vehicles = sum(lane_counts)
        signal_times[:] = [
            max(10, min(int(count / total_vehicles * 60), 60)) if total_vehicles > 0 else 10
